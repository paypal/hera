...
import com.paypal.jmux.util.NetStringConstants;
...
public class NetstringWriter{
	...
	public void add(byte[] _cmd, long _data) throws OccIOException {
		try {
			int pos2 = OccJdbcConverter.long2occ(_data, tmp_buff2);
			int pos = OccJdbcConverter.int2occ(_cmd.length + (TMP_BUFF_LEN - pos2) + 1, tmp_buff);
	...
	public void add(byte[] _cmd, byte[] _data) throws OccIOException {
		try {
			int pos = OccJdbcConverter.int2occ(_cmd.length + _data.length + 1, tmp_buff);
			baos.write(tmp_buff, pos, TMP_BUFF_LEN - pos);
			baos.write(NetStringConstants.CHARACTER_COLON);
			baos.write(_cmd);
			baos.write(NetStringConstants.CHARACTER_SPACE);
	...
	public void add(byte[] _cmd) throws OccIOException {
		try {
			int pos = OccJdbcConverter.int2occ(_cmd.length, tmp_buff);
			baos.write(tmp_buff, pos, TMP_BUFF_LEN - pos);
			baos.write(NetStringConstants.CHARACTER_COLON);
			baos.write(_cmd);
			baos.write(NetStringConstants.CHARACTER_COMMA);
	...
	public void flush() throws IOException {
		if(cmd_count ==1) {
		    ...
		    byte[] data = baos .toByteArray();
            int pos = OccJdbcConverter.int2occ(1 + 1 + data. length, tmp_buff);
