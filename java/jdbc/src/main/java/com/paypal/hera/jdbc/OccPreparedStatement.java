...
import com.paypal.hera.ex.OccClientException;
import com.paypal.hera.ex.OccExceptionBase;
import com.paypal.hera.ex.OccIOException;
import com.paypal.hera.ex.OccInternalErrorException;
...
import com.paypal.hera.util.OccStatementsCache.ShardingInfo;

public class OccPreparedStatement extends OccStatement implements PreparedStatement {
	
	final static Logger LOGGER = LoggerFactory.getLogger(OccPreparedStatement.class);
	...
	protected OccJdbcConverter converter; 
	private ArrayList<Pair<BindType, ArrayList<byte[]> > > batched_in_params;
	private ArrayList<Integer> max_batched_value_size;
	
	public OccPreparedStatement(OccConnection occConnection, String _sql) {
		super(occConnection);
		sql = _sql;
		in_params = new HashMap<Integer, Pair<BindType, byte[]> >();
		converter = occConnection.getConverter();
		batched_in_params = new ArrayList<Pair<BindType, ArrayList<byte[]> > >();
		max_batched_value_size = new ArrayList<Integer>();
		if (LOGGER.isDebugEnabled())
			LOGGER.debug("OccPreparedStatement created, id: " + this.hashCode() + ", conn id: " + occConnection.hashCode() + ", SQL: " + _sql);
	}

	protected void prepare() throws OccExceptionBase {
		super.prepare(sql);
	}
	...
	protected void bindIn(Map<Integer, Pair<BindType, byte[]> > params) throws OccExceptionBase {
		if (params.size() > stCache.getParamCount())
			throw new OccSQLException("Illegal number of bind parameters, trying to bind " + params.size() + " instead of " + stCache.getParamCount());

		for (Entry<Integer, Pair<BindType, byte[]>> entry : params.entrySet()){
			connection.getOccClient().bind(stCache.actualParamName(entry.getKey()), 
	...
	protected void bindAndShardInfo() throws SQLException {
		...
		if (connection.shardingEnabled()) {
			if (connection.getShardID() == -1) {
				...
				if (null != stCache.getShardingInfo()) {
					connection.getOccClient().reset();
					throw new OccClientException("Before query: '" + sql + "' the shard hint was not reset");
				}
				if (null != connection.getShardKeyPayload()) {
					connection.getOccClient().reset();
					throw new OccClientException("For query: '" + sql + "' the shard hint was not reset before setting new hint");
				}
			}
		}
		bindIn(in_params);
		if (shardKeyPayload != null) {
			if (LOGGER.isDebugEnabled())
				LOGGER.debug("OccPreparedStatement (id: " + this.hashCode() + ") shard key set");
			connection.getOccClient().shardKey(shardKeyPayload);
		}
	}
	...
	public void addBatch() throws SQLException {
		...
		if (in_params.size() != batched_in_params.size()) {
			throw new OccSQLException("Illegal number of varbinds: " + batched_in_params.size() + " and " + in_params.size() + ". Each row needs to have the same number of varbinds.");
		}
	...
	public int[] executeBatch() throws SQLException {
		...
		for (int i = 0; i < batched_in_params.size(); i++) {
			connection.getOccClient().bindArray(stCache.actualParamName(i + 1), max_batched_value_size.get(i), 
		...
		try {
			...
			if (LOGGER.isDebugEnabled())
				LOGGER.debug("OccPreparedStatement::executeBacth(): " + rows + " rows changed");
		} catch (SQLException ex) {
			if (batch_size > 1) {
				String errorMessage = ex.getMessage().substring(Consts.OCC_SQL_ERROR_PREFIX.length());
				int updateCounts[] = OccJdbcUtil.getArrayCounts(errorMessage, batch_size);
	...
	* is to get everything in the clob and pop it into a string.
	...
	public ResultSetMetaData getMetaData() throws SQLException {
		checkOpened();
		throw new OccSQLException("MetaData is available from ResultSet only");
	}
	...
	protected final void bindString(int _index, byte[] _value) throws SQLException {
		in_params.put(Integer.valueOf(_index), new Pair<BindType, byte[]>(BindType.OCC_TYPE_STRING, _value));
	}

	protected final void bindDateTime(int _index, byte[] _value) throws SQLException {
		in_params.put(Integer.valueOf(_index), new Pair<BindType, byte[]>(BindType.OCC_TYPE_TIMESTAMP, _value));
	}

	protected final void bindDateTimeTZ(int _index, byte[] _value) throws SQLException {
		in_params.put(Integer.valueOf(_index), new Pair<BindType, byte[]>(BindType.OCC_TYPE_TIMESTAMP_TZ, _value));
	}
	...
	public void setNull(int _index, int sqlType) throws SQLException {
		...
		case Types.VARCHAR:
			...
			break;
		case Types.BLOB:
			in_params.put(Integer.valueOf(_index), new Pair<BindType, byte[]>(BindType.OCC_TYPE_BLOB_SINGLE_ROUND, NULL_VAL));
			break;
		case Types.CLOB:
			in_params.put(Integer.valueOf(_index), new Pair<BindType, byte[]>(BindType.OCC_TYPE_CLOB_SINGLE_ROUND, NULL_VAL));
			break;
		case Types.VARBINARY:
		case Types.LONGVARBINARY:
			in_params.put(Integer.valueOf(_index), new Pair<BindType, byte[]>(BindType.OCC_TYPE_RAW, NULL_VAL));
			break; 
		case Types.DATE:
		case Types.TIME:
...
//			unsupportedParamType("Type.sql." + sqlType);
	...
	public void setInt(int index, int x) throws SQLException {
		checkParamStart();
		bindString(index, OccJdbcConverter.int2occ(x));
	}

	public void setBoolean(int index, boolean x) throws SQLException {
		checkParamStart();
		bindString(index, OccJdbcConverter.int2occ(x ? 1 : 0));
	}
	...
	public void setShort(int index, short x) throws SQLException {
		checkParamStart();
		bindString(index, OccJdbcConverter.short2occ(x));
	}

	public void setLong(int index, long x) throws SQLException {
		checkParamStart();
		bindString(index, OccJdbcConverter.long2occ(x));
	}

	public void setFloat(int index, float x) throws SQLException {
		checkParamStart();
		bindString(index, OccJdbcConverter.float2occ(x));
	}

	public void setDouble(int index, double x) throws SQLException {
		checkParamStart();
		bindString(index, OccJdbcConverter.double2occ(x));
	}
	...
	public void setDate(int index, Date x) throws SQLException {
		...
		checkParamStart();
		try {
			bindDateTime(index, converter.date2occ(x));
		} catch (UnsupportedEncodingException e) {
			throw new OccSQLException("Failed to encode Date", e);
		}
	}
	...
	public void setTime(int index, Time x) throws SQLException {
		...
		checkParamStart();
		try {
			bindDateTime(index, converter.time2occ(x));
		} catch (UnsupportedEncodingException e) {
			throw new OccSQLException("Failed to encode Time", e);
		}
	}
	...
	public void setTimestamp(int index, Timestamp x) throws SQLException
	{
		...
		checkParamStart();
		try {
			bindDateTime(index, converter.timestamp2occ(x));
		} catch (UnsupportedEncodingException e) {
			throw new OccSQLException("Failed to encode Timestamp", e);
		}
	}
		...
		throws SQLException
	{
		...
		checkParamStart();
		try {
			bindDateTimeTZ(index, converter.date2occ(x, cal));
		} catch (UnsupportedEncodingException e) {
			throw new OccSQLException("Failed to encode Timestamp", e);
		}
	}
		...
		throws SQLException
	{
		...
		checkParamStart();
		try {
			bindDateTimeTZ(index, converter.time2occ(x, cal));
		} catch (UnsupportedEncodingException e) {
			throw new OccSQLException("Failed to encode Timestamp", e);
		}
	}
	...
	public void setTimestamp(int index, Timestamp x, Calendar cal) throws SQLException
	{
		...
		checkParamStart();
		try {
			bindDateTimeTZ(index, converter.timestamp2occ(x, cal));
		} catch (UnsupportedEncodingException e) {
			throw new OccSQLException("Failed to encode Timestamp", e);
		}
	}
		...
		Reader reader, int length) throws SQLException
	{
		...
		while (totalCharsRead < length) {
			...
			if (charsRead == -1)
				throw new OccRuntimeException("Cannot read input stream to bind");
		}
		try {
			in_params.put(Integer.valueOf(index), new Pair<BindType, byte[]>(BindType.OCC_TYPE_CLOB_SINGLE_ROUND, OccJdbcConverter.string2occ(new String(chars))));
		} catch (UnsupportedEncodingException e) {
			throw new OccSQLException("Can't encode char stream");
		}
	}
		...
		throws SQLException
	{
		...
		in_params.put(Integer.valueOf(index), new Pair<BindType, byte[]>(BindType.OCC_TYPE_BLOB_SINGLE_ROUND, bytes));
	}
	...
	public void setBytes(int index, byte[] value) throws SQLException {
		...
		checkParamStart();
		in_params.put(Integer.valueOf(index), new Pair<BindType, byte[]>(BindType.OCC_TYPE_RAW, value));
	}
			...
			if (parameterAsNum instanceof java.math.BigDecimal) {
				...
				try {
					...
					try {
						...
						throw new OccClientException("Can't set scale of '" + scale + "' for DECIMAL argument '" + parameterAsNum + "'", "42000");
					}
				}
			...
			try {
				...
				case Types.BOOLEAN:
					...
					if (x instanceof Boolean) {
						...
						throw new OccClientException("No conversion from " + x.getClass().getName() + " to Types.BOOLEAN possible.", "4200");
					}
				...
				throw new OccClientException("Exception ", ex);
			}
		}
	}
	...
	public void setObject(int index, Object x) throws SQLException
	{
		if (x == null) {
		  ...
		  setDate(index, (Date) x);
			  ...
			  // Can't infer a type.
			throw new OccSQLException("Can''t infer the SQL type in setObject for " + x.getClass().getName());
		}
	}
	...
	public void setBlob(int index, Blob x) throws SQLException {
		...
		checkParamStart();
		in_params.put(Integer.valueOf(index), new Pair<BindType, byte[]>(BindType.OCC_TYPE_BLOB_SINGLE_ROUND, x.getBytes(1, (int)x.length())));
	}
	...
	public void setClob(int index, Clob x) throws SQLException {
		...
		checkParamStart();
		try {
			in_params.put(Integer.valueOf(index), new Pair<BindType, byte[]>(BindType.OCC_TYPE_CLOB_SINGLE_ROUND, OccJdbcConverter.string2occ(x.getSubString(1, (int)x.length()))));
		} catch (UnsupportedEncodingException e) {
			throw new OccSQLException("Can't encode clob");
		}
	}
		...
		throws SQLException
	{
		...
		checkParamStart();
		bindString(index, OccJdbcConverter.BigDecimal2occ(x));
	}
	...
	public void setURL(int parameterIndex, URL x)
		throws SQLException
	{
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.setURL is not implemented");
	}

	public ParameterMetaData getParameterMetaData()
		throws SQLException
	{
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.getParameterMetaData is not implemented");
	}

	// JDBC 4.0 
	public void setAsciiStream(int parameterIndex, InputStream x) throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.setAsciiStream is not implemented");
	}
	...
	public void setBinaryStream(int parameterIndex, InputStream x) throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.setBinaryStream is not implemented");
	}
	...
	public void setBinaryStream(int parameterIndex, InputStream x, long length) throws SQLException {
		...
		if (length > Integer.MAX_VALUE)
			throw new OccSQLException("InputStream too large");
        ...
        try {
			...
			throw new OccSQLException("IO exception reading Reader", e);
		}
	}

	public void setBlob(int parameterIndex, InputStream inputStream) throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.setBlob is not implemented");
	}
	...
	public void setCharacterStream(int parameterIndex, Reader reader, long length) throws SQLException {
		...
		if (length > Integer.MAX_VALUE)
			throw new OccSQLException("Reader too large");
	...
	public void setClob(int parameterIndex, Reader reader) throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.setClob is not implemented");
	}
	...
	public void setNCharacterStream(int parameterIndex, Reader value) throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.setNCharacterStream is not implemented");
	}
	...
	public void setNClob(int parameterIndex, NClob value) throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.setNClob is not implemented");
	}
	...
	public void setNString(int parameterIndex, String value) throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.setNString is not implemented");
	}

	public void setRowId(int parameterIndex, RowId x) throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.setRowId is not implemented");
	}

	public void setSQLXML(int parameterIndex, SQLXML xmlObject) throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.setSQLXML is not implemented");
	}

	public boolean isClosed() throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.isClosed is not implemented");
	}

	public boolean isPoolable() throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.isPoolable is not implemented");
		
	}

	public void setPoolable(boolean poolable) throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.setPoolable is not implemented");
		
	}

	public boolean isWrapperFor(Class<?> iface) throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.isWrapperFor is not implemented");
		
	}

	public <T> T unwrap(Class<T> iface) throws SQLException {
		throw new SQLFeatureNotSupportedException("OCCPreparedStatement.unwrap is not implemented");
	...
	public byte[] shardProcessing(String _shard_key, ArrayList<Integer> _shard_key_bind_pos, ArrayList<Integer> _scuttle_id_pos) throws SQLException {
		...
		if (_shard_key_bind_pos.size() != _scuttle_id_pos.size()) {
			throw new OccInternalErrorException("shard_key_vec.size() != scuttle_id_vec.size()");
		}
		try {
			...
			for (int i = 0; i < _shard_key_bind_pos.size(); i++) {
				...
				baos.write(_data);
				// scuttle id
				setInt(_scuttle_id_pos.get(i), OccJdbcUtil.getScuttleID(_data));
			}
			return baos.toByteArray();
		} catch (IOException ex) {
			throw new OccIOException(ex);
		}
	}
