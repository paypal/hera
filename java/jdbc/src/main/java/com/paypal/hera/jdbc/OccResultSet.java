...
import com.paypal.hera.client.OccClient;
...
public class OccResultSet implements ResultSet {

	private boolean closed;
	private OccConnection connection; 
	private OccClient occ; 
	private OccStatement stmt;
	...
	public OccResultSet(OccConnection _connection, OccStatement occStatement, OccClient _occ, int _fetchSize) throws SQLException {
		...
		isFetchDone = false;
		paramWasNull = false;
		converter = ((OccConnection)(occStatement.getConnection())).getConverter();
		columnIndexes = occStatement.getColumnIndexes();
		fetchNext(false);
	}
	...
	void fetchAllData() throws OccExceptionBase {
		while (!isFetchDone) {
			fetchNext(true/*append new fetch data*/);
		}
	}
	
	private void helperClose(boolean nice) throws OccExceptionBase {
		if (closed) {
			return;
		}
		closed = true;
	...
	void hardClose() throws OccExceptionBase {
		helperClose(false);
	}
	...
	private void checkOpened() throws OccRuntimeException {
		if (closed)
			throw new OccRuntimeException("OccResultSet is closed");
	}
	...
	public void setFetchSize(int rows) throws SQLException {
		checkOpened();
		throw new OccSQLException("Fetch size can be changed on statement only");
	}
	...
	public ResultSetMetaData getMetaData() throws SQLException {
		...
		if ((connection.supportResultSetMetaData()) && (metaData == null)) {
			OccResultSetMetaData occmetaData = new OccResultSetMetaData(stmt.getColumnMeta());	
			occmetaData.setPayloadSizeInBytes(getPayloadSizeInByteForCurrentFetch());
	...
	public int findColumn(String columnName) throws SQLException {
		...
		if (columnIndexes == null)
			throw new OccClientException("Cannot get column name, please check the config for properties " + OCCClientConfigHolder.SUPPORT_COLUMN_NAMES_PROPERTY +
					" or " + OCCClientConfigHolder.SUPPORT_COLUMN_INFO_PROPERTY);
	...
	public void setFetchDirection(int direction) throws SQLException {
		if (direction != FETCH_FORWARD) {
			throw new SQLFeatureNotSupportedException("Occ resultset is forward-only");
		} else {
			checkOpened();
		}
	}
	...
	private boolean moveNext() throws OccExceptionBase {
	...
	private void fetchNext(boolean append) throws OccExceptionBase {
		try {
			ArrayList<ArrayList<byte[]>> new_data = occ.fetch(fetchSize);
			if (append)
	...
	public int getRow() throws SQLException {
		throw new SQLFeatureNotSupportedException("Occ resultset is forward-only"); // always throw
	}
	...
	public int getInt(int columnIndex) throws SQLException {
		...
		if (paramWasNull)
			return 0;
		return OccJdbcConverter.occ2int(value);
	}
	...
	public String getString(int columnIndex) throws SQLException {
		...
		return OccJdbcConverter.occ2String(value);
	}
	...
	public long getLong(int columnIndex) throws SQLException {
		...
		if (paramWasNull)
			return 0;
		return OccJdbcConverter.occ2long(value);
	}
	...
	public float getFloat(int columnIndex) throws SQLException {
		...
		if (paramWasNull)
			return 0;
		return OccJdbcConverter.occ2float(value);
	}
	...
	public double getDouble(int columnIndex) throws SQLException {
		...
		if (paramWasNull)
			return 0;
		return OccJdbcConverter.occ2double(value);
	}
	...
	public Timestamp getTimestamp(int columnIndex) throws SQLException {
		...
		if (paramWasNull)
			return null;
		try {
			return converter.occ2timestamp(value);
		...
		throws SQLException
	{
		...
		if (paramWasNull)
			return null;
		try {
			return converter.occ2timestamp(value, cal);
		} catch (ParseException e) {
			throw new OccSQLException("Failed to parse Timestamp", e);
		}
	}
	...
	public Time getTime(int columnIndex) throws SQLException {
		...
		if (paramWasNull)
			return null;
		try {
			return converter.occ2time(value);
		} catch (ParseException e) {
	...
	public Date getDate(int columnIndex) throws SQLException {
		...
		if (paramWasNull)
			return null;
		try {
			return converter.occ2date(value);
		} catch (ParseException e) {
	...
	public short getShort(int columnIndex) throws SQLException {
		...
		if (paramWasNull)
			return 0;
		return OccJdbcConverter.occ2short(value);
	}
	...
	public Blob getBlob(int columnIndex) throws SQLException {
		...
		if (paramWasNull)
			return null;
		return OccJdbcConverter.occ2Blob(value);
	}
	...
	public Clob getClob(int columnIndex) throws SQLException {
		...
		if (paramWasNull)
			return null;
		return OccJdbcConverter.occ2Clob(value);
	}
	...
	public Reader getCharacterStream(int columnIndex) throws SQLException {
		...
		if (paramWasNull)
			return null;
		return new StringReader(OccJdbcConverter.occ2String(value));
	}
	...
	public BigDecimal getBigDecimal(int columnIndex) throws SQLException {
		...
		if (paramWasNull)
			return null;
		return OccJdbcConverter.occ2BigDecimal(value);
	}
	...
	public String getCursorName() throws SQLException {
		throw new SQLFeatureNotSupportedException("Update operations are not supported on Occ resultset");
	}
