// Test application for mux using the Mux driver for Golang's database/sql.
//
...
import (
	"flag"
	_ "github.com/paypal/hera/gomuxdriver/muxtcp" /*to register the driver*/
	"github.com/paypal/hera/utility/logger"

	"context"
	"database/sql"
	"fmt"
	//    "occmux-go/occclient" /* to cast to the Occ extension */
	...
	"github.com/paypal/hera/gomuxdriver" /* to cast to the Occ extension */
)

var url = flag.String("url", "", "OCCs URL, default to the Url from the input file")
...
func runSelect(ctx context.Context, txn *sql.Tx, q *Query, c *sql.Conn) error {
	if q.ShardKey != "" {
		occ := gomuxdriver.InnerConn(c)
		occ.SetShardKeyPayload(q.ShardKey)
		defer occ.ResetShardKeyPayload()
	}
