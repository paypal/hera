...
func cfg() (map[string]string, map[string]string, testutil.WorkerType) {
	...
	appcfg["readonly_children_pct"] = "50"
        ...
        appcfg["request_backlog_timeout"] = "150"
        //appcfg["soft_eviction_probability"] = "80"
        appcfg["short_backlog_timeout"] = "40"
        appcfg["idle_timeout_ms"] = "600000"
        appcfg["lifo_short_backlog_que"] = "false"

	opscfg := make(map[string]string)
	opscfg["opscfg.default.server.max_connections"] = "16"
...
func TestReadNormalWriteBklgEviction(t *testing.T) {
	fmt.Println ("TestReadNormalWriteBklgEviction begin +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
	logger.GetLogger().Log(logger.Debug, "TestReadNormalWriteBklgEviction begin +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")

	testutil.RunDML("insert into test_simple_table_1 (ID, Name, Status) VALUES (12346, 'Smith', 100)")
	var id_num int
        fmt.Println ("Having 17 threads to do fetch, insert, sleep & commit")
	...
	fmt.Println ("Verify BKLG & bklg_timeout events, check that timeout is correct as specified in config")
	count := testutil.RegexCountFile ("WARNING.*bklg0_timeout.*timeout 150", "cal.log");
        if (count < 2) {
            t.Fatalf ("Error: expected 2 or more bklg0_timeout events");
        }
        count = testutil.RegexCountFile ("WARNING.*bklg0_eviction.*timeout 150", "cal.log");
        if (count < 1) {
	...
	logger.GetLogger().Log(logger.Debug, "TestReadNormalWriteBklgEviction done  -------------------------------------------------------------")
}
