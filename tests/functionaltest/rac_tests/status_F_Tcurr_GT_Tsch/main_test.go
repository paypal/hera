...
/* #####################################################################################
 #  Testing RAC change to status 'F' when scheduled time < current time but 
 #  schedule_time > worker start time
 #  Verify mux detects status change and restart workers
 #######################################################################################*/

func TestStatusF_Tcurr_GT_Tsch(t *testing.T) {
	fmt.Println ("TestStatusF_Tcurr_GT_Tsch begin +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
	logger.GetLogger().Log(logger.Debug, "TestStatusF_Tcurr_GT_Tsch begin +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
	...
	//Insert a row with status F and scheduled time 5 seconds less than current time, scheduled time > worker start time
        time.Sleep(5 * time.Second)
        err = testutil.SetRacNodeStatus ("F", "hera-test", -5) 
        if err != nil {
	...
	logger.GetLogger().Log(logger.Debug, "TestStatusF_Tcurr_GT_Tsch done  -------------------------------------------------------------")
}
