...
/* #####################################################################################
 ...
 #  Tcurr : The current time when check point is being executed in occworker
 #  Tsch  : It is the scheduled time in the table when the DBA is expecting the transition
 #  Tstart: This is the start time when the occworker process started running)
 #  Verify mux detects status change 'R' in hera_maint table when rac maint schedule time < current time
 #  and schedule time > worker start time 
 #######################################################################################*/

func TestStatusU_to_R_Tcurr_GT_Tsch2(t *testing.T) {
	fmt.Println ("TestStatusU_to_R_Tcurr_GT_Tsch2 begin +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
	logger.GetLogger().Log(logger.Debug, "TestStatusU_to_R_Tcurr_GT_Tsch2 begin +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
	...
	//Set Rac status "U" with current time < schedule time and schedule time > worker start time
        time.Sleep(5 * time.Second)
        err = testutil.SetRacNodeStatus ("R", "hera-test",  -5)
        if err != nil {
	...
	if ( testutil.RegexCountFile("E.*RACMAINT_INFO_CHANGE.*R.*0", "cal.log") > count) {
	...
	logger.GetLogger().Log(logger.Debug, "TestStatusU_to_R_Tcurr_GT_Tsch2 done  -------------------------------------------------------------")
}
