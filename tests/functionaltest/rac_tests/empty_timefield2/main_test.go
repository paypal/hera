...
/* #####################################################################################
 #  In hera_maint table, insert first rac row with time and second row without time
 #  Verify mux ignores the second row and processes successfully the first row in hera_maint table 
 #######################################################################################*/

func TestEmptyTime2(t *testing.T) {
	...
	fmt.Println ("Insert a row to table")
        ...
        err = testutil.InsertRacEmptyTime2 ("R", "hera-test",  1)
        if err != nil {
	...
	count2  := testutil.RegexCountFile("E.*RACMAINT_INFO_CHANGE.*R.*0", "cal.log")
        if ( count2 > count) {
	...
	logger.GetLogger().Log(logger.Debug, "TestEmptyTime2 done  -------------------------------------------------------------")
}
