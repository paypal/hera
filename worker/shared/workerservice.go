...
const envModule string = "OCC_NAME"
...
func Start(adapter CmdProcessorAdapter) {
	cfg, err := config.NewTxtConfig("occ.txt")
	if err != nil {
		fmt.Printf("Can't open config occ.txt")
		return
	}
	...
	err = logger.CreateLogger("occ.log", logPrefix, int32(logLevel))
	if err != nil {
		return
	}
	//
	...
	evt := cal.NewCalEvent(cal.EventTypeServerInfo, "occworker-go-start", cal.TransOK, "")
	evt.Completed()
	//
	// set up uds.
	//
...
outerloop:
	for {
		...
		case sig, ok = <-sigchannel:
			if sig == signalRecover {
				...
				evt := cal.NewCalEvent("OCCWORKER", "recoverworker", cal.TransOK, "")
				evt.Completed()
				//
		...
		if logger.GetLogger().V(logger.Verbose) {
			logger.GetLogger().Log(logger.Verbose, sockMux.Name(), ": occworker read <<<", DebugString(ns.Serialized))
		}
		//
	...
	if logger.GetLogger().V(logger.Info) {
		logger.GetLogger().Log(logger.Info, "======== occworker exits")
	}
	sockMux.Close()
	sockMux = nil
}
