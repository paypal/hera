...
// Package muxtls provides the Mux driver for Go's database/sql package.
//
// The driver should be used via the database/sql package:
//
//  import "database/sql"
//  import _ "github.com/paypal/hera/gomuxdriver/muxtls"
//
...
// OccTLSDrv is a global object keeping the configuration
var OccTLSDrv *occDriver // TODO make it private, create the tlsConfig in Open, where parameters like InsecureSkipVerify are passed via the URL

func init() {
	OccTLSDrv = &occDriver{
		TLSCfg: &tls.Config{},
	}
	sql.Register("occ", OccTLSDrv)
}

func (driver *occDriver) Open(url string) (driver.Conn, error) {
	ipport := url[2:]
	if logger.GetLogger().V(logger.Info) {
		logger.GetLogger().Log(logger.Info, "Dialing to occ server:", ipport)
	}
	conn, err := tls.Dial("tcp", ipport, OccTLSDrv.TLSCfg)
