...
func newStmt(occ *occConnection, sql string) *stmt {
	st := &stmt{occ: occ, fetchChunkSize: []byte("0")}
	...
	if logger.GetLogger().V(logger.Debug) {
		logger.GetLogger().Log(logger.Debug, occ.id, "final SQL:", st.sql)
	}
	return st
}
...
func (st *stmt) Exec(args []driver.Value) (driver.Result, error) {
	sk := 0
	if len(st.occ.shardKeyPayload) > 0 {
		sk = 1
	}
	crid := 0
	if st.occ.corrID != nil {
		crid = 1
	}
	binds := len(args)
	...
	for _, val := range args {
		...
		if logger.GetLogger().V(logger.Verbose) {
			logger.GetLogger().Log(logger.Verbose, st.occ.id, "Bind name =", string(nss[idx-1].Payload), ", value=", string(nss[idx].Payload))
		}
		idx++
	}
	if sk == 1 {
		nss[idx] = netstring.NewNetstringFrom(common.CmdShardKey, st.occ.shardKeyPayload)
		idx++
	}
	nss[idx] = netstring.NewNetstringFrom(common.CmdExecute, nil)
	cmd := netstring.NewNetstringEmbedded(nss)
	err := st.occ.execNs(cmd)
	if err != nil {
		return nil, err
	}
	ns, err := st.occ.getResponse()
	if err != nil {
		return nil, err
	}
	if ns.Cmd != common.RcValue {
		...
		case common.RcError:
			return nil, fmt.Errorf("Internal occ error: %s", string(ns.Payload))
		default:
	...
	if logger.GetLogger().V(logger.Debug) {
		logger.GetLogger().Log(logger.Debug, st.occ.id, "DML successfull, rows affected:", res.nRows)
	}
	return res, nil
}
...
Loop:
		...
		if ns.Cmd != common.RcValue {
			...
			case common.RcStillExecuting:
				if logger.GetLogger().V(logger.Info) {
					logger.GetLogger().Log(logger.Info, st.occ.id, " Still executing ...")
				}
				// continues the loop
			case common.RcSQLError:
				return nil, fmt.Errorf("SQL error: %s", string(ns.Payload))
	...
	if logger.GetLogger().V(logger.Debug) {
		logger.GetLogger().Log(logger.Debug, st.occ.id, "Query successfull, num columns:", cols)
	}
	return newRows(st.occ, cols, st.fetchChunkSize)
}
...
// implementing the extension OccStmt interface
