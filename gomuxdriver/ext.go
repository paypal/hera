...
//OccConn is an API extension for sql.Conn
type OccConn interface {
...
// OccStmt is an API extension for *sql.Stmt
type OccStmt interface {
	// A hit to the server for how many rows to return at once
	SetFetchSize(num int)
}

// InnerConn returns a OccConn interface from a *sql.Conn. It uses reflection to walk the internal structure of *sql.Conn to
// return the inner structure implemented in the driver
func InnerConn(c *sql.Conn) OccConn {
	nilv := reflect.Value{}
	...
	return (*drvConn).(*occConnection)
}

// InnerStmt returns a OccStmt interface from a *sql.Stmt. It uses reflection to walk the internal structure of *sql.Stmt to
// return the inner structure implemented in the driver
func InnerStmt(st *sql.Stmt) OccStmt {
	nilv := reflect.Value{}
