...
func newRows(occ *occConnection, cols int, fetchChunkSize []byte) (*rows, error) {
	rs := &rows{occ: occ, cols: cols, currentRow: 0, fetchChunkSize: fetchChunkSize}
	err := rs.fetchResults()
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		case common.RcNoMoreData:
			if logger.GetLogger().V(logger.Verbose) {
				logger.GetLogger().Log(logger.Verbose, r.occ.id, "Rows: cols = ", r.cols, ", numValues =", len(r.vals))
			}
			r.completed = true
			return nil
		}
...
func (r *rows) Columns() []string {
	// TODO using occ column names command
...
func (r *rows) Next(dest []driver.Value) error {
	if logger.GetLogger().V(logger.Verbose) {
		//		logger.GetLogger().Log(logger.Verbose, r.occ.id, "Rows.Next(): currentRow = ", r.currentRow, ", numValues =", len(dest))
	}
