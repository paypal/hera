...
type StateLog struct {
	...
	//
	mWorkerStates [](map[OccWorkerType]([][]*WorkerStateInfo))
	...
	//
	mConnStates [](map[OccWorkerType]([]*ConnStateInfo))
	...
	//
	mTypeTitles [](map[OccWorkerType]([]string))
	//
	// header row (state)
	//
	mStateHeader string
	...
	//
	//
	mLastReqCnt [](map[OccWorkerType]([]int64))
	mLastRspCnt [](map[OccWorkerType]([]int64))
...
func (sl *StateLog) GetTotalConnections() int {
	...
	for s := 0; s < sl.maxShardSize; s++ {
		...
		for t := 0; t < int(wtypeTotalCount); t++ {
			instCnt := len(sl.mWorkerStates[s][OccWorkerType(t)])
			for n := 0; n < instCnt; n++ {
				//
				// do not count close state.
				//
				for c := Backlog; c < Close; c++ {
					counter += sl.mConnStates[s][OccWorkerType(t)][n].perStateCnt[c]
				}
			} // instance
		} // wtype
	} // sharding
...
func (sl *StateLog) GetStrandedWorkerCountForPool(shardID int, wType OccWorkerType, instID int) int {
	var cnt = 0
	//
...
func (sl *StateLog) init() error {
	...
	//
	sl.mWorkerStates = make([]map[OccWorkerType][][]*WorkerStateInfo, sl.maxShardSize)
	sl.mConnStates = make([]map[OccWorkerType][]*ConnStateInfo, sl.maxShardSize)
	sl.mTypeTitles = make([]map[OccWorkerType][]string, sl.maxShardSize)
	sl.mLastReqCnt = make([]map[OccWorkerType][]int64, sl.maxShardSize)
	sl.mLastRspCnt = make([]map[OccWorkerType][]int64, sl.maxShardSize)
	//
	// for each shard, initialize map
	//
	for s := 0; s < sl.maxShardSize; s++ {
		sl.mWorkerStates[s] = make(map[OccWorkerType][][]*WorkerStateInfo, wtypeTotalCount)
		sl.mConnStates[s] = make(map[OccWorkerType][]*ConnStateInfo, wtypeTotalCount)
		sl.mTypeTitles[s] = make(map[OccWorkerType][]string, wtypeTotalCount)
		sl.mLastReqCnt[s] = make(map[OccWorkerType][]int64, wtypeTotalCount)
		sl.mLastRspCnt[s] = make(map[OccWorkerType][]int64, wtypeTotalCount)
		//
		// for each workertype, initialize two dimension array
		//
		for t := 0; t < int(wtypeTotalCount); t++ {
			instCnt := workerpoolcfg[s][OccWorkerType(t)].instCnt
			workerCnt := workerpoolcfg[s][OccWorkerType(t)].maxWorkerCnt

			sl.mWorkerStates[s][OccWorkerType(t)] = make([][]*WorkerStateInfo, instCnt)
			sl.mConnStates[s][OccWorkerType(t)] = make([]*ConnStateInfo, instCnt)
			sl.mTypeTitles[s][OccWorkerType(t)] = make([]string, instCnt)
			sl.mLastReqCnt[s][OccWorkerType(t)] = make([]int64, instCnt)
			sl.mLastRspCnt[s][OccWorkerType(t)] = make([]int64, instCnt)
			//
			// for each "standby" instance, initialize array for each worker
			//
			for i := 0; i < instCnt; i++ {
				sl.mWorkerStates[s][OccWorkerType(t)][i] = make([]*WorkerStateInfo, workerCnt)
				sl.mConnStates[s][OccWorkerType(t)][i] = &ConnStateInfo{}
				sl.mConnStates[s][OccWorkerType(t)][i].perStateCnt = make([]int, MaxConnState)

				sl.mLastReqCnt[s][OccWorkerType(t)][i] = 0
				sl.mLastRspCnt[s][OccWorkerType(t)][i] = 0
				//
				...
				for w := 0; w < workerCnt; w++ {
					...
					sl.mWorkerStates[s][OccWorkerType(t)][i][w] = &WorkerStateInfo{state: OccWorkerStatus(wsInit)}
				}
			}
		}
	}
 ...
 */
func (sl *StateLog) resizeWorkers(s int, t OccWorkerType, i int, newSize int) error {
	//
	// if not initialized with s/t/i, it is an error.
	//
	var wtStates = sl.mWorkerStates[s][OccWorkerType(t)]
	var connStates = sl.mConnStates[s][OccWorkerType(t)]
	...
	if currentSize < newSize {
		for a := currentSize; a < newSize; a++ {
			wtStates[i] = append(wtStates[i], &WorkerStateInfo{state: OccWorkerStatus(wsInit)})
		}
	} else {
 ...
 */
func (sl *StateLog) updateConnectionState(_shardID int, _type OccWorkerType, _instID int, _old ConnState, _new ConnState) {
	if logger.GetLogger().V(logger.Verbose) {
	...
	connState := sl.mConnStates[_shardID][OccWorkerType(_type)][_instID]
	//
	...
	if _new == Idle || _new == Assign {
		for s := 0; s < sl.maxShardSize; s++ {
			for t := 0; t < int(wtypeTotalCount); t++ {
				instCnt := len(sl.mConnStates[s][OccWorkerType(t)])
				for n := 0; n < instCnt; n++ {
					sl.mConnStates[s][OccWorkerType(t)][n].perStateCnt[_new]++
				}
			}
		}
	} else {
	...
	if _old == Idle || _old == Assign {
		for s := 0; s < sl.maxShardSize; s++ {
			for t := 0; t < int(wtypeTotalCount); t++ {
				instCnt := len(sl.mConnStates[s][OccWorkerType(t)])
				for n := 0; n < instCnt; n++ {
					sl.mConnStates[s][OccWorkerType(t)][n].perStateCnt[_old]--
				}
			}
		}
	} else {
...
func (sl *StateLog) setWorkerState(_shardID int, _type OccWorkerType, _instID int, _workerID int, _newState OccWorkerStatus) {
	if logger.GetLogger().V(logger.Verbose) {
	...
	//
	var wtStates = sl.mWorkerStates[_shardID][OccWorkerType(_type)]
	if _workerID >= len(wtStates[_instID]) {
		return
	}

	workerState := sl.mWorkerStates[_shardID][OccWorkerType(_type)][_instID][_workerID]
...
func (sl *StateLog) genReport() {
	...
	for s := 0; s < sl.maxShardSize; s++ {
		for t := 0; t < int(wtypeTotalCount); t++ {
			...
			//
			for n := 0; n < instCnt; n++ {
				workerCnt := len(sl.mWorkerStates[s][OccWorkerType(t)][n])
				if workerCnt == 0 {
					continue
				}
				//
				...
				//
				var buf bytes.Buffer
				buf.WriteString(fmt.Sprintf("%-11s", sl.mTypeTitles[s][OccWorkerType(t)][n]))
				//
				// collect worker states.
				//
				for w := 0; w < workerCnt; w++ {
					workerState := sl.mWorkerStates[s][OccWorkerType(t)][n][w]
				...
				for c := 0; c < MaxConnState; c++ {
					stateCnt[MaxWorkerState+c] = sl.mConnStates[s][OccWorkerType(t)][n].perStateCnt[c]
				...
				//
				hb := cal.NewCalHeartBeat("STATE", sl.mTypeTitles[s][OccWorkerType(t)][n], cal.TransOK, "")
				for i := 0; i < (MaxWorkerState + MaxConnState - 1); i++ {
					buf.WriteString(fmt.Sprintf("%6d", stateCnt[i]))
					hb.AddDataInt(StateNames[i], int64(stateCnt[i]))
				}
				hb.AddDataInt("req", int64(reqCnt-sl.mLastReqCnt[s][OccWorkerType(t)][n]))
				hb.AddDataInt("resp", int64(respCnt-sl.mLastRspCnt[s][OccWorkerType(t)][n]))
				/*
				...
				*/
				...
				sl.mLastReqCnt[s][OccWorkerType(t)][n] = reqCnt
				sl.mLastRspCnt[s][OccWorkerType(t)][n] = respCnt
			} // instance
		} // wtype
	} // sharding
}
