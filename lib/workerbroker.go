...
// OccWorkerType defines the possible worker type
type OccWorkerType int

// constants for OccWorkerType
const (
	wtypeRW OccWorkerType = iota
	wtypeRO               // @TODO
	wtypeStdBy
	wtypeTotalCount
)
...
type WorkerBroker struct {
	...
	//            of that OccWorkerType. wtypeStdBy has multiples instances known
	//            as multiple standbys. RW will have two instances for primary/failover.
	// workerpools [shard](map[OccWorkerType][inst]*WorkerPool)
	//
	workerpools []map[OccWorkerType][]*WorkerPool
	poolCfgs    []map[OccWorkerType]*WorkerPoolCfg
	//
...
// GetWorkerBrokerInstance returns the singleton broker instance where different request handler goroutines can use to get a free occworker
...
func (broker *WorkerBroker) init() error {
	...
	//
	broker.workerpools = make([](map[OccWorkerType][]*WorkerPool), broker.maxShardSize)
	broker.poolCfgs = make([](map[OccWorkerType]*WorkerPoolCfg), broker.maxShardSize)
	var workercnt int
	for s := 0; s < broker.maxShardSize; s++ {
		//
		// setup broker configuration, inst and worker size can be loaded from cdb
		//
		broker.poolCfgs[s] = make(map[OccWorkerType]*WorkerPoolCfg, wtypeTotalCount)
		//
		...
		//
		broker.workerpools[s] = make(map[OccWorkerType][]*WorkerPool, wtypeTotalCount)
		for t := 0; t < int(wtypeTotalCount); t++ {
			poolcfg := broker.poolCfgs[s][OccWorkerType(t)]
			if logger.GetLogger().V(logger.Verbose) {
				logger.GetLogger().Log(logger.Verbose, "init pool ", poolcfg)
			}
			workercnt += (poolcfg.instCnt * poolcfg.maxWorkerCnt)
			broker.workerpools[s][OccWorkerType(t)] = make([]*WorkerPool, poolcfg.instCnt)
			for i := 0; i < poolcfg.instCnt; i++ {
				broker.workerpools[s][OccWorkerType(t)][i] = &WorkerPool{}
			}
		}
	}
...
func (broker *WorkerBroker) RestartWorkerPool(_moduleName string) error {
	...
	for s := 0; s < broker.maxShardSize; s++ {
		for t := 0; t < int(wtypeTotalCount); t++ {
			poolcfg := broker.poolCfgs[s][OccWorkerType(t)]
			for i := 0; i < poolcfg.instCnt; i++ {
				err = broker.workerpools[s][OccWorkerType(t)][i].Init(OccWorkerType(t), poolcfg.maxWorkerCnt, i, s, _moduleName)
				if err != nil {
...
func (broker *WorkerBroker) GetWorkerPoolCfgs() (pCfgs []map[OccWorkerType]*WorkerPoolCfg) {
...
func (broker *WorkerBroker) GetWorkerPool(wType OccWorkerType, ids ...int) (workerbroker *WorkerPool, err error) {
	//
		...
		for {
			...
			case signal := <-sigchannel:
				...
				case syscall.SIGCHLD:
					...
					// get none-occ defunct processes. +1 in case racing casue mapsize=0.
					//
...
*/
func (broker *WorkerBroker) resizePool(wType OccWorkerType, maxWorkers int, shardID int) {
