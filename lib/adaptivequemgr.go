 ...
 * anyway, we are not blocking processing path of the occmux. the entire logic in this manager
 ...
 * but we assume external client follows standard occ ns protocol to not sending new sql before
 * all the results are received.
 *
 * this struct is not thread safe. the way it works in occmux is that each workerpool has one
		...
		if mgr.shouldRecover() {
			...
			if ok {
				if workerclient != nil {
					...
					if logger.GetLogger().V(logger.Warning) {
						logger.GetLogger().Log(logger.Warning, "occproxy saturation recover: sql will be terminated in child", workerclient.pid, "while assigned to client, close client connection", workerclient.Type)
					}
				} else {
					//
