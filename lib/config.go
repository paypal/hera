...
// InitConfig initializes the configuration, both the static configuration (from occ.txt) and the dynamic configuration
func InitConfig() error {
	cdb, err := config.NewTxtConfig("occ.txt")
	if err != nil {
		return err
	}
	...
	logFile := cdb.GetOrDefaultString("log_file", "occ.log")
	...
	if err != nil {
		if logger.GetLogger().V(logger.Info) {
			logger.GetLogger().Log(logger.Info, "failed to get num_standby_dbs from occ.txt, defaulting to 0")
		}
		//
	...
	var numWorkers int
	numWorkers = 6
	//err = config.InitOpsConfigWithName("../opscfg/occ.txt")
	...
	gAppConfig.ManagementTablePrefix = cdb.GetOrDefaultString("management_table_prefix", "")
...
func (cfg *Config) GetBacklogLimit(wtype OccWorkerType, shard int) int {
	if wtype == wtypeRO {
