...
type WorkerPool struct {
	...
	Type    OccWorkerType // the worker type like write, read
	InstID  int
...
func (pool *WorkerPool) Init(wType OccWorkerType, size int, instID int, shardID int, moduleName string) error {
	pool.Type = wType
...
func (pool *WorkerPool) RacMaint(racReq racAct) {
	...
	pool.poolCond.L.Unlock()
	// TODO: C++ worker logs one event for each worker, in the occworker, so
