...
type shardInfo struct {
	...
	sessionShardID int // shard ID set manualy via OCC_SET_SHARD_ID
...
/**
processSetShardId handles OCC_SET_SHARD_ID command from the client to set the shard ID to be used for the following requests.
The shard ID remains set until is reset via this command with shard id equal -1.
OCC_SET_SHARD_ID is used for the rare cases when a table is not sharded. In fact if later a query is attempted on a sharded table,
	...
	for i := 1; i < sz; i++ {
		if (requests[i].Cmd == common.CmdBindName) && crd.isShardKey(string(requests[i].Payload)) {
			...
			if requests[i].Cmd == common.CmdShardKey {
				if crd.shard.sessionShardID != -1 {
					// OCC_SET_SHARD_ID and ShardKey not allowed simultaneously
					evt := cal.NewCalEvent(EvtTypeSharding, EvtNameShardIDAndKey, cal.TransOK, "")
					evt.AddDataInt("sql", int64(uint32(crd.sqlhash)))
					evt.Completed()
					return true, errors.New("Unsupported both OCC_SET_SHARD_ID and ShardKey")
				}
