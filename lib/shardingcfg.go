...
func getSQL() string {
	...
	if len(GetConfig().ShardingPostfix) != 0 {
		return fmt.Sprintf("SELECT scuttle_id, shard_id, read_status, write_status from %socc_shard_map_%s where status = 'Y'", GetConfig().ManagementTablePrefix, GetConfig().ShardingPostfix)
	}
	...
	if err == nil {
		...
		if err == nil {
			buf[n] = 0
			sql := fmt.Sprintf("SELECT scuttle_id + usleep(%s) - %s, shard_id, read_status, write_status from %socc_shard_map where status = 'Y'", string(buf), string(buf), GetConfig().ManagementTablePrefix)
	...
	return fmt.Sprintf("SELECT scuttle_id, shard_id, read_status, write_status from %socc_shard_map where status = 'Y'", GetConfig().ManagementTablePrefix)
}
...
func getWLSQL() string {
	if len(GetConfig().ShardingPostfix) != 0 {
		return fmt.Sprintf("SELECT shard_key, shard_id, read_status, write_status FROM %socc_whitelist_%s WHERE enable = 'Y'", GetConfig().ManagementTablePrefix, GetConfig().ShardingPostfix)
	}
	return fmt.Sprintf("SELECT shard_key, shard_id, read_status, write_status FROM %socc_whitelist WHERE enable = 'Y'", GetConfig().ManagementTablePrefix)
}
