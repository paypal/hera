...
public class HeraStatement implements Statement {
	...
	private ResultSet lastInsertIdPtr;
	private LastInsertIdResultSet lastInsertIdInst = new LastInsertIdResultSet();
	private static HeraResultSet lastInsertIdEmpty = new HeraResultSet();
	...
	protected void prepare(String _sql) throws HeraExceptionBase {
		...
		lastInsertIdPtr = lastInsertIdEmpty;
	}
	...
	protected int helperExecuteUpdate(boolean _add_commit) throws SQLException {
		try {
			...
			// if no bind out & if more data, fetch last insert id
			if (countOutParameter() <= 0 && connection.getHeraClient().packetHasMoreData()) {
				ArrayList<ArrayList<byte[]> > result = connection.getHeraClient().fetchOutBindVars(1);
				lastInsertIdPtr = lastInsertIdInst;
				lastInsertIdInst.setLong(Long.parseLong(new String(result.get(0).get(0), "UTF-8")));
			}
	...
	protected int countOutParameter() { return -1; }
