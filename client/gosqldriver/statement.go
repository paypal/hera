...
func (st *stmt) Exec(args []driver.Value) (driver.Result, error) {
	...
	nss, err := prepareExecNetstrings(st, convertValueToNamedValue(args), crid, sk)
	if err != nil {
		return nil, err
	}
	...
	nRows, err := handleExecResponse(st)
	if err != nil {
		return nil, err
	}
	if logger.GetLogger().V(logger.Debug) {
		logger.GetLogger().Log(logger.Debug, st.hera.getID(), "DML successfull, rows affected:", nRows)
	}
	return &result{nRows: nRows}, nil
}
...
func (st *stmt) ExecContext(ctx context.Context, args []driver.NamedValue) (driver.Result, error) {
	...
	nss, err := prepareExecNetstrings(st, args, crid, sk)
	if err != nil {
		return nil, err
	}
	cmd := netstring.NewNetstringEmbedded(nss)
...
func (st *stmt) Query(args []driver.Value) (driver.Rows, error) {
	...
	nss, err := prepareQueryNetstrings(st, convertValueToNamedValue(args), crid, sk)
	if err != nil {
		return nil, err
	}
	...
	cols, err := handleQueryResponse(st)
	if err != nil {
		return nil, err
	}

	if logger.GetLogger().V(logger.Debug) {
		logger.GetLogger().Log(logger.Debug, st.hera.getID(), "Query successful, num columns:", cols)
	}
...
func (st *stmt) QueryContext(ctx context.Context, args []driver.NamedValue) (driver.Rows, error) {
	...
	nss, err := prepareQueryNetstrings(st, args, crid, sk)
	if err != nil {
		return nil, err
	}
	cmd := netstring.NewNetstringEmbedded(nss)
...
func prepareQueryNetstrings(st *stmt, args []driver.NamedValue, crid, sk int) ([]*netstring.Netstring, error) {
	binds := len(args)
...
func handleQueryResponse(st *stmt) (int, error) {
	...
	for {
		...
		if ns.Cmd != common.RcValue {
			...
			case common.RcSQLError:
				return 0, fmt.Errorf("SQL error: %s", string(ns.Payload))
			case common.RcError:
				return 0, fmt.Errorf("Internal hera error: %s", string(ns.Payload))
			default:
				return 0, fmt.Errorf("Unknown code: %d, data: %s", ns.Cmd, string(ns.Payload))
			}
		} else {
			break
		}
	...
	if ns.Cmd != common.RcValue {
		return 0, fmt.Errorf("Unknown code2: %d, data: %s", ns.Cmd, string(ns.Payload))
	}
...
func prepareExecNetstrings(st *stmt, args []driver.NamedValue, crid, sk int) ([]*netstring.Netstring, error) {
	binds := len(args)
...
func handleExecResponse(st *stmt) (int, error) {
	ns, err := st.hera.getResponse()
	if err != nil {
		return 0, err
	}
	...
	nRows, err := strconv.Atoi(string(ns.Payload))
	if err != nil {
		return 0, err
	}
	return nRows, nil
